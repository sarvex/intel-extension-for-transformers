...
from intel_extension_for_transformers.transformers.trainer import NLPTrainer as Trainer
...
class OptimizationArguments:
    ...
    nas: bool = field(
        default=False,
        metadata={"help": "Whether or not to apply NAS."},
    )
    approach: str = field(
        default="basic",
        metadata={
            "help": "Which NAS approach to use."
        },
    )
    search_algorithm: str = field(
        default="BO",
        metadata={
            "help": "Search algorithm for NAS."
        },
    )
    max_trials: int = field(
        default=100,
        metadata={
            "help": "Maximum trials for NAS."
        },
    )
...
def main():
    ...
    # Auto Distillation
    if optim_args.nas:
        if 'mobilebert' in model_args.model_name_or_path:
            # for MobileBERT
            stages = 24
            nas_config = NASConfig(
                    ...
                    approach=optim_args.approach,
                    search_algorithm=optim_args.search_algorithm,
        ...
        elif 'bert-tiny' in model_args.model_name_or_path:
            # for BERT-Tiny
            nas_config = NASConfig(
                    search_space={
        ...
        best_model_archs = trainer.nas(
            nas_config,
            model_cls=model_cls
        )
        print("Best model architectures obtained by NAS are as follow.")
